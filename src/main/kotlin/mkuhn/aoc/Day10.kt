...
fun day10part2(input: List<String>): Int {
    ...
    val pathPositions = path.map { p -> p.position }
    val containedAlongPath =
        path.map { pipe -> pipe.fromDirection.getInwardClockwise().moveToward(pipe.position) }
            .toSet()
            .filter { it !in pathPositions }
    val containedAlongPath2 =
        path.map { pipe -> pipe.fromDirection.getInwardCounterClockwise().moveToward(pipe.position) }
            .toSet()
            .filter { it !in pathPositions }
    //containedAlongPath.forEach { println("$it : ${grid.get(it)}") }
    //containedAlongPath2.forEach { println("$it : ${grid.get(it)}") }
...
fun List<List<Char>>.getPositionsInDirectionWhile(start: Pair<Int, Int>, dir: Direction,
                                                  predicate: (Pair<Int, Int>) -> Boolean): List<Pair<Int, Int>> =
    generateSequence(start) { pos -> dir.moveToward(pos) }.takeWhile(predicate).toList()
